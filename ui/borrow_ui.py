...
def setUp_borrow_books_frame(borrow_frame, shared_data, browse_books_frame):
    ...
    header_label.place(x=0, y=0, relwidth=1, height=50)  


    book_details_frame = tk.Frame(borrow_frame, bd=2, relief="ridge", bg="white")
    book_details_frame.place(x=150, y=70, width=1100, height=250)  
    ...
    user_details_frame.place(x=150, y=370, width=1100, height=250) 
    ...
    username_entry.insert(0, shared_data.get_user_id())  
    ...
    populate_details_button = tk.Button(borrow_frame,text="Populate Details",font=("Arial", 14),bg="#1A8F2D",fg="white",width=20, height=2,command=lambda: populate_book_details(book_list_tree, shared_data))
    populate_details_button.place(x=650, y=250, width=200, height=40)


    confirm_borrow_button = tk.Button(borrow_frame,text="Confirm Borrow",font=("Arial", 14),bg="#1A8F2D",fg="white",width=20, height=2,command=lambda: verify_user_and_borrow(username_entry, password_entry, shared_data))
    confirm_borrow_button.place(x=650, y=550, width=200, height=40)

    # Back button
    back_button = tk.Button(borrow_frame,text="Back",font=("Arial", 14),bg="#1A8F2D",fg="white",width=20, height=2,command=lambda: show_frame(browse_books_frame))
