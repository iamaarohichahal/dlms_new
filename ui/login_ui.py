...
def setUp_Login(login_frame, register_frame, user_dashboard_frame, admin_dashboard_frame, shared_data):
    ...
    user_login_button = tk.Button(login_frame, text="User Login", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white", width=20, height=2, command=lambda: login_user(simpledialog.askstring("User Login", "Enter username:"),simpledialog.askstring("User Login", "Enter password:", show='*'),user_dashboard_frame,shared_data))
    user_login_button.place(relx=0.5, rely=0.4, anchor='center')

    # Button for Admin Login
    admin_login_button = tk.Button(login_frame, text="Admin Login", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white", width=20, height=2,command=lambda: login_admin(simpledialog.askstring("Admin Login", "Enter username:"),simpledialog.askstring("Admin Login", "Enter password:", show='*'),admin_dashboard_frame))
    admin_login_button.place(relx=0.5, rely=0.5, anchor='center')

    # Button to Register (Show Register Frame)
    register_button = tk.Button(login_frame, text="Register", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white", width=20, height=2, command=lambda: show_frame(register_frame))
