...
def setUp_loan_details(loan_details_frame, user_dashboard_frame, shared_data, loan_return_frame):
    ...
    return_button = tk.Button(loan_details_frame, text="Return Book", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white",width=button_width, height=button_height, relief="solid",command=lambda: handle_return_book(loan_details_tree, loan_return_frame, loan_details_frame, shared_data))
    return_button.place(relx=0.25, rely=0.9, anchor='center')

    back_button = tk.Button(loan_details_frame, text="Back", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white",width=button_width, height=button_height, relief="solid",command=lambda: show_frame(user_dashboard_frame))
    back_button.place(relx=0.5, rely=0.9, anchor='center')

    view_loans_button = tk.Button(loan_details_frame, text="View Loans", font=("Arial", 16, "bold"), bg="#4CAF50", fg="white",width=button_width, height=button_height, relief="solid",command=lambda: view_loans(shared_data, loan_details_tree))
