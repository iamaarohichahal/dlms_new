...
def setUp_loan_return_frame(loan_return_frame, show_frame, loan_details_frame, shared_data):
    ...
    book_details_frame.place(relx=0.5, rely=0.25, anchor="center", width=800, height=250) 
    ...
    book_list_tree.column('Title', anchor=tk.W, width=380)  
    ...
    book_list_tree.pack(pady=20, padx=15, fill="both")  
    ...
    populate_details_button.pack(pady=10)  
    ...
    confirm_return_button = tk.Button(loan_return_frame,text="Confirm Return",font=("Arial", 14),bg="#1A8F2D",fg="white",width=20, height=2,command=lambda: verify_user_and_return(username_entry, password_entry, shared_data))
    confirm_return_button.place(relx=0.5, rely=0.68, anchor='center')


    back_button = tk.Button(loan_return_frame,text="Back",font=("Arial", 14),bg="#1A8F2D",fg="white",width=20, height=2,command=lambda: show_frame(loan_details_frame))
